...
class PostgreSQLItemService implements ItemService {
  ...
  async createItem(listId: ID, item: IItem): Promise<IItem> {
    const query = `INSERT INTO item (list_id, description, checked) VALUES ($1, $2, $3) RETURNING _id as id, list_id as listId, description, checked`;
    const result = await this.database.query(query, [listId, item.description, item.checked]);
  ...
  async changeItemDescription(listId: ID, id: ID, description: string): Promise<IItem> {
    const query = `UPDATE item SET description = $1 WHERE list_id = $2 AND _id = $3 RETURNING _id as id, list_id as listId, description, checked`;
  ...
  async changeItemChecked(listId: ID, id: ID, checked: boolean): Promise<IItem> {
    const query = `UPDATE item SET checked = $1 WHERE list_id = $2 AND _id = $3 RETURNING _id as id, list_id as listId, description, checked`;
